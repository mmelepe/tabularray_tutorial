...
\begin{Code}[show-output]
\begin{tblr}{
	\highLight{width=0.7\linewidth,}  % 70 % of the line width. One can also input a value in cm, pt...
	colspec={ll|X|X|},
}
	Furniture & Price & Shop     & Comment \\ \hline
	Bed       & 150   & MOBILI   & An entry-level, without mattress. \\
...
Another neat feature of the \snippet{X} columns is the fact that they accept negative coefficients. A negative value means that the \snippet{X} column will expand \emph{at most} to the given coefficient, if the text in the column requires that much space, and if there are other \snippet{X} cells that will use the remaining space.

\begin{Code}[show-output]
\begin{tblr}{|X|X[-2.5]|} \hline 
...
Let us now explain the \snippet{\textbackslash SetCell} command. It takes an optional argument between square brackets \snippet{[]} and a mandatory one between curly braces \snippet{\{\}}:

\begin{itemize}
	\item \snippet{[r=2]} indicates that the current cell will span over 2 \textbf{r}ows.
...
We can also merge columns together, with the optional argument \snippet{[c=4]} (here, merging over 4 columns). Finally, one can merge both rows and columns together:
...
\begin{infobox}{TL;DR}
	\begin{itemize}
		...
		\item Columns of type \snippet{X} fill all the available space. Their size can be adjusted as follows: \snippet{X[2]}. It can receive other arguments: \snippet{X[2,c,m]}. A negative coefficient gives a maximum expansion for the \snippet{X} column: \snippet{X[-1.5]}.
...
\vspace*{\fill} % \vfill not working zt the beginning of a page because latex ignores any space at the beginning of a page
